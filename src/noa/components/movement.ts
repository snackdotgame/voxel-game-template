...
export class MovementState {
  ...
  declare moveForce: number;
  declare responsiveness: number;
  declare runningFriction: number;
  declare standingFriction: number;

  declare airMoveMult: number;
  declare jumpImpulse: number;
  declare jumpForce: number;
  declare jumpTime: number;
  declare airJumps: number;

  declare _jumpCount: number;
  declare _currjumptime: number;
  declare _isJumping: boolean;
...
// `noa` is typed loosely: this module is imported by the shared server-safe
// sim (src/shared/sim.ts), so it must not import the engine even type-only.
export default function (noa: any) {
  return {
    ...
    system: function movementProcessor(dt: number, states: any[]) {
...
export function applyMovementPhysics(dt: number, state: MovementState, body: RigidBody) {
