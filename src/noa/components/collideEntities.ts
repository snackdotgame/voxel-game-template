...
export default function (noa: Engine) {
  var intervals: (number[] | Float32Array)[] = [];
  ...
  return {
    ...
    state: {
      ...
      collideBits: 1 | 0,
      collideMask: 1 | 0,
      callback: null as ((otherId: number) => void) | null,
    },
    ...
    system: function entityCollider(dt: number, states: any[]) {
      var ents = noa.ents;
      ...
      boxIntersect(intervals, function (a: number, b: number) {
        var stateA = states[a];
        var stateB = states[b];
  ...
  function handleCollision(noa: Engine, stateA: any, stateB: any) {
    var idA = stateA.__id;
    var idB = stateB.__id;
  ...
  function cylindricalHitTest(
    stateA: any,
    stateB: any,
    intervalA: number[] | Float32Array,
    intervalB: number[] | Float32Array,
  ) {
    if (stateA.cylinder) {
      if (stateB.cylinder) {
  ...
  function cylinderCylinderTest(a: number[] | Float32Array, b: number[] | Float32Array) {
  ...
  function cylinderBoxTest(cyl: number[] | Float32Array, cube: number[] | Float32Array) {
