...
// `body` is typed with its post-onAdd (always-populated) shape, matching the
// d.ts the engine used to publish; it holds null only before onAdd runs.
...
export default function (noa: Engine) {
  return {
    ...
    onAdd: function (entID: number, state: any) {
    ...
    onRemove: function (entID: number, state: any) {
