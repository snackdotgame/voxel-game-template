...
export class Engine extends EventEmitter {
  ...
  worldName!: string;
  /** Multiplier for how fast time moves. */
  timeScale!: number;
  ...
  /** The default listener for block highlighting, kept here in case
   * people want to remove it later. */
  ...
  /** @internal */
  _targetedBlockDat!: { blockID: number; position: any; normal: any; adjacent: any };
  /** @internal */
  _prevTargetHash!: number;
  /** @internal */
  _pickPos!: any;
  /** @internal */
  _pickResult!: { _localPosition: any; position: number[]; normal: number[] };
  /** @internal */
  vec3?: any;
  /** @internal */
  ndarray?: any;
   ...
   * @internal
   */
  render(dt: number, framePart: number) {
    dt *= this.timeScale || 1;
   ...
   */
  setBlock(id: number, x: any, y = 0, z = 0) {
   ...
   */
  addBlock(id: number, x: any, y = 0, z = 0) {
   ...
   */
  globalToLocal(global: number[], globalPrecise: number[] | null, local: number[]) {
   ...
   */
  localToGlobal(local: number[], global: number[], globalPrecise: number[] | null = null) {
    var off = this.worldOriginOffset;
    if (globalPrecise) {
      for (var i = 0; i < 3; i++) {
   ...
   *
   * @param pos where to pick from (default: player's eye pos)
   * @param dir direction to pick along (default: camera vector)
   * @param dist pick distance (default: `noa.blockTestDistance`)
   * @param blockTestFunction which voxel IDs can be picked (default: any solid voxel)
   */
  pick(
    pos: number[] | null = null,
    dir: number[] | null = null,
    dist: number | null = -1,
    blockTestFunction: ((id: number) => boolean) | null = null,
  ) {
  ...
  _localPick(
    ...
    var off = this.worldOriginOffset;
    var testVoxel = function (x: number, y: number, z: number) {
 ...
 *
 */
function checkWorldOffset(noa: Engine) {
...
function deprecateStuff(noa: Engine) {
  var ver = `0.27`;
  var dep = (loc: any, name: string, msg: string) => {
    var throwFn = () => {
