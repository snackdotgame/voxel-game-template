...
export class Container extends EventEmitter {
  ...
  constructor(noa: Engine, opts: any) {
    ...
    // shell listeners
    this._shell.onPointerLockChanged = (hasPL: boolean) => {
...
function getOrCreateCanvas(el: HTMLElement) {
...
function detectPointerLock(self: Container) {
 ...
 */
function doCanvasBugfix(noa: Engine, canvas: HTMLCanvasElement) {
  var ct = 0;
  var fixCanvas = () => {
    var w = canvas.width;
    canvas.width = w + 1;
