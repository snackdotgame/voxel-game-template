...
export class World extends EventEmitter {
   ...
   * ```
   */
  chunkSortingDistFn: (i: number, j: number, k: number) => number;
  ...
  /** @internal */
  _addDistanceFn: ((i: number, j: number, k: number) => boolean) | null;
  /** @internal */
  _remDistanceFn: ((i: number, j: number, k: number) => boolean) | null;
  /** @internal */
  ...
  /** @internal */
  _prevSortingFn: ((i: number, j: number, k: number) => number) | null;
  /** @internal */
  ...
  /** @internal */
  _coordsToChunkIndexes: (this: World, x: number, y: number, z: number) => number[];
  /** @internal */
  _coordsToChunkLocals: (this: World, x: number, y: number, z: number) => number[];
  /** @internal */
   ...
   * @param id - the string specified when the chunk was requested
   * @param array - an ndarray of voxel data
   * @param userData - an arbitrary value for game client use
   * @param fillVoxelID - specify a voxel ID here if you want to signify that
   ...
   */
  setChunkData(id: string, array: any, userData: any = null, fillVoxelID = -1) {
   ...
   */
  setAddRemoveDistance(addDist: number | number[] = 2, remDist: number | number[] = 3) {
  ...
  tick() {
    ...
    var done1 = false;
    var done2 = false;
    var done3: boolean | undefined = false;
    while (t < t1) {
      if (!done1) {
  ...
  /** @internal */
  _queueChunkForRemesh(chunk: Chunk) {
 ...
 */
function getPlayerChunkIndexes(world: World) {
 ...
 */
function findChunksToRequest(world: World, ci: number, cj: number, ck: number) {
...
var checkOneLocation = (world: World, i: number, j: number, k: number) => {
 ...
 */
function findDistantChunksToRemove(world: World, ci: number, cj: number, ck: number) {
 ...
 */
function findChunksToMesh(world: World) {
  var maxQueued = 10;
 ...
 */
function invalidateChunksInBox(world: World, box: any) {
 ...
 */
function markAllChunksInvalid(world: World) {
 ...
 */
function processRequestQueue(world: World) {
  var toRequest = world._chunksToRequest;
...
function processRemoveQueue(world: World) {
 ...
 */
function processMeshingQueue(world: World, firstOnly: boolean): boolean | undefined {
...
function possiblyQueueChunkForMeshing(world: World, chunk: any) {
 ...
 */
function requestNewChunk(world: World, i: number, j: number, k: number) {
 ...
 * If userData is passed in it will be attached to the chunk
 */
function setChunkData(world: World, reqID: string, array: any, userData: any, fillVoxelID: number) {
  var arr = reqID.split("|");
 ...
 */
function removeChunk(world: World, i: number, j: number, k: number) {
  var chunk = world._storage.getChunkByIndexes(i, j, k);
...
function doChunkRemesh(world: World, chunk: Chunk) {
...
function chunkCoordsToIndexesGeneral(this: World, x: number, y: number, z: number) {
  var cs = this._chunkSize;
  return [Math.floor(x / cs) | 0, Math.floor(y / cs) | 0, Math.floor(z / cs) | 0];
}
function chunkCoordsToLocalsGeneral(this: World, x: number, y: number, z: number) {
  var cs = this._chunkSize;
  var i = (x % cs) | 0;
  if (i < 0) i += cs;
  var j = (y % cs) | 0;
...
function chunkCoordsToIndexesPowerOfTwo(this: World, x: number, y: number, z: number) {
  var shift = this._coordShiftBits;
  return [(x >> shift) | 0, (y >> shift) | 0, (z >> shift) | 0];
}
function chunkCoordsToLocalsPowerOfTwo(this: World, x: number, y: number, z: number) {
 ...
 */
function sortQueueByDistanceFrom(
  world: World,
  queue: LocationQueue,
  pi?: number,
  pj?: number,
  ...
  var localDist = (i: number, j: number, k: number) => distFn(pi! - i, pj! - j, pk! - k);
  if (pi === undefined) {
    [pi, pj, pk] = getPlayerChunkIndexes(world);
  }
  queue.sortByDistance(localDist, reverse);
}
var defaultSortDistance = (i: number, j: number, k: number) => i * i + j * j + k * k;

/**
 * keep neighbor data updated when chunk is added or removed
 */
function updateNeighborsOfChunk(world: World, ci: number, cj: number, ck: number, chunk: any) {
...
function makeDistanceTestFunction(xsize: number, ysize: number) {
  var asq = xsize * xsize;
  var bsq = ysize * ysize;
  // spherical case
  if (xsize === ysize) return (i: number, j: number, k: number) => i * i + j * j + k * k <= asq;
...
function _report(world: World, name: string, arr: number[][], ext?: any) {
  var full = 0,
    empty = 0,
    exist = 0,
    surrounded = 0,
...
var profile_hook = makeProfileHook(PROFILE_EVERY, "world ticks:", 1 as any) as (
  state: string,
) => void;
var profile_queues_hook = ((every: number): ((state: string, world?: World) => void) => {
  if (!(every > 0)) return () => {};
  var iter = 0;
  var counts: { [key: string]: number } = {};
  var queues: { [key: string]: number } = {};
  ...
  return function profile_queues_hook(state, world) {
    ...
    queues.tomesh =
      (queues.tomesh || 0) + world!._chunksToMesh.count() + world!._chunksToMeshFirst.count();
