...
export class TerrainMesher {
  ...
  constructor(noa: Engine) {
     ...
     */
    this.meshChunk = function (this: TerrainMesher, chunk, ignoreMaterials = false) {
      profile_hook("start");
...
class GreedyMesher {
  mesh: (chunk: Chunk, ignoreMaterials: boolean) => { [key: string]: MeshedFaceData };

  constructor(noa: Engine, terrainMatManager: TerrainMatManager) {
    ...
    var fakeGetTerrainID = (_matID: number) => 1;
    var terrainIDgetter = realGetTerrainID;

    /**
     * Entry point
     *
     * @returns keyed by terrain material ID
     */
    this.mesh = function (chunk, ignoreMaterials) {
      ...
      var faceDataSet: { [key: string]: MeshedFaceData } = {};
      faceDataPool.reset();
      ...
      for (var d = 0; d < 3; ++d) {
        ...
        var nabVoxelsArr = chunk._neighbors.data.map((c: any) => {
          if (c && c.voxels) return c.voxels.transpose(d, u, v);
          return null;
        });

        // ndarray of the previous, similarly transposed
        var nabVoxelsT = ndarray(nabVoxelsArr, [3, 3, 3]).lo(1, 1, 1).transpose(d, u, v);
     ...
     */
    function prepareSolidityLookup(nabVoxelsT: any, size: number) {
    ...
    function voxelIsSolid(i: number, j: number, k: number) {
     ...
     *
     * @returns number of mesh faces found
     */

    function constructMeshMask(
      d: number,
      arrA: any,
      iA: number,
      arrB: any,
      iB: number,
      wholeLayerVoxel: number[] | null = null,
    ) {
    ...
    function constructGeometryFromMasks(
      ...
      len2: number,
      numFaces: number,
      faceDataSet: { [key: string]: MeshedFaceData },
    ) {
      var doAO = noa.rendering.useAO;
      var mask = maskCache;
      var aomask = aoMaskCache;
...
class MeshBuilder {
  ...
  constructor(noa: Engine, terrainMatManager: TerrainMatManager) {
    ...
    function addAtlasIndices(indArr: Float32Array, faceNum: number, atlasIndex: number) {
    ...
    function decideTriDir(A: number, B: number, C: number, D: number) {
    ...
    function pushMeshColors_noAO(colors: Float32Array, faceNum: number, col: number[]) {
 ...
 *
 */
function unpackAOMask(aomask: number): [number, number, number, number] {
  var A = aomask & 3;
  var B = (aomask >> 2) & 3;
  var D = (aomask >> 4) & 3;
  var C = (aomask >> 6) & 3;
...
var profile_hook: (state: string) => void = PROFILE_EVERY
