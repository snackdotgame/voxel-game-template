...
export class SceneOctreeManager {
  ...
  addMesh: (mesh: AbstractMesh, isStatic: boolean, pos: number[], chunk: any) => void;
  removeMesh: (mesh: AbstractMesh) => void;
  setMeshVisibility: (mesh: AbstractMesh, visible?: boolean) => void;

  /** @internal */
  constructor(rendering: Rendering, blockSize: number) {
    ...
    var octree = new Octree<any>(NOP!);
    scene._selectionOctree = octree;
    octree.blocks = [];
    var octBlocksHash: { [key: number]: any } = {};
    ...
    this.addMesh = (mesh, isStatic, pos, _chunk) => {
