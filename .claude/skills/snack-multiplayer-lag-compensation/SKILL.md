---
name: snack-multiplayer-lag-compensation
description: Implement bounded server-side lag compensation in generated Snack.Game projects. Use for authoritative shooters or other latency-sensitive games where instant actions such as hitscan fire or melee traces should be validated against recent target history without trusting client-reported hits, rewinding the live world, or granting unbounded latency advantage.
---
...
- `snack-build-multiplayer` and the selected primary netcode skill
- `src/client.ts`, `src/server.ts`, and the shared message definitions
- generated `snack:client` and `snack:server` types
- [references/example.md](references/example.md) for a strict TypeScript hitscan example
