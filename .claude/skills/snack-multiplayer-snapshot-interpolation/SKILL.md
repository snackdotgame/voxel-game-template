---
name: snack-multiplayer-snapshot-interpolation
description: Implement authoritative snapshot synchronization and remote interpolation in generated Snack.Game projects. Use for continuous real-time games, especially non-deterministic physics or simulations where the server owns state and clients need smooth presentation without full replay or rollback.
---
...
Use this skill after `snack-build-multiplayer` selects snapshot synchronization, especially when the
...
- use `snack-multiplayer-client-prediction` only for a subsystem with a defined correction path
