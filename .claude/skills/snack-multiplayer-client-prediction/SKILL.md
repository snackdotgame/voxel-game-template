---
name: snack-multiplayer-client-prediction
description: Implement responsive local client prediction and authoritative reconciliation in generated Snack.Game projects. Use when local controls must react before a server round trip, while the server remains authoritative and remote entities use snapshots/interpolation.
---
...
- `snack-multiplayer-snapshot-interpolation` for remote entity presentation

Use this skill after `snack-build-multiplayer` selects local prediction.
...
Use `snack-multiplayer-rollback` only when late inputs must rewind a wider simulation and the
relevant simulation passes deterministic replay tests.
